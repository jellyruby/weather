...
export function gsapStart() {
    ...
    const refreshCtrl = document.querySelector('.preview > .preview__refresh');
    const detailItem = document.querySelector('.detail__item');
    const cellImages = document.querySelectorAll('.cell__img-inner');
    ...
    cellImages.forEach(obj => {obj.addEventListener('click',() => {
        if ( isAnimating ) return;
        isAnimating = true;
        ...
        .to(refreshCtrl, {
            opacity: 1,
            onComplete: () => refreshCtrl.classList.add('preview__close--show')
        }, 'start')
        .addLabel('end', 0)
        .to(detailItem, {
            opacity: 1,
            onComplete: () => detailItem.classList.add('detail__item--show')
        }, 'end')
    ...
    refreshCtrl?.addEventListener('click',()=>{
        if ( isAnimating ) return;
        isAnimating = true;
        ...
        .to(detailItem, {
            opacity: 0 ,
            onComplete: () => detailItem.classList.remove('detail__item--show')
        }, 'prestart')
        .addLabel('start', 0)
        .to([row.DOM.images, row.previewItem.DOM.images], {
            scale: 1,
